...
| `crates/shared/scheduler-engine/src/software_states.rs` | **Canonical** `load_software_states_for_tenant` implementation (single source of truth) |
| `crates/ui/web-api-queries/src/queries/mqtt_software_states.rs` | Re-exports `load_software_states_for_tenant` from `scheduler-engine`; adds `AgentConnectivityInfo` + `load_agent_connectivity_for_tenant` |
