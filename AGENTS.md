...
`object_id` = `uptrakit_{fn_slug}_{item_slug}`.

Host summary entities: unique_id `uptrakit_{t}_{h}_pkgs`,
entity name `"{friendly_name} packages"`, `object_id` = `uptrakit_{fn_slug}_packages`.
...
`object_id` = `uptrakit_{fn_slug}_security_updates`. Install triggers a `security_only = true` batch.
