...
│   │   │   └── proxmox/              # uptrakit-plugin-infrastructure-proxmox                (lib)  — Proxmox VE infrastructure plugin: controller-side REST API client for PVE, VM/CT discovery, manual host matching, extension manifests (proxmox.hosts page + proxmox.host-info panel), extension action handlers; DB table: proxmox_host_mappings; agent-side modules: pve_setup (PVE detection + API credential creation via pveum), guest_exec (command execution inside LXC/QEMU guests via pct exec / qm guest exec)
│   │   ├── releases/
