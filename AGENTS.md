...
- `EnrollPayload` includes required `host_info: HostInfo` field (no `csr_pem` — keypair generation is deferred until after approval). The `agent_id` is generated by the controller.
