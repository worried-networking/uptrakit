...
>
	{#if successMessage}
		<aside
			...
			aria-live="polite"
			aria-atomic="true"
			class="pointer-events-auto rounded-lg p-4 shadow-lg preset-filled-success-500"
		>
			<p>{successMessage}</p>
		</aside>
	{/if}
	...
	{#if errorMessage}
		<aside
			...
			aria-live="assertive"
			aria-atomic="true"
			class="pointer-events-auto flex items-center justify-between gap-4 rounded-lg p-4 shadow-lg preset-filled-error-500"
		>
			<p>{errorMessage}</p>
	...
	{#each alerts as alert (alert.id)}
		<aside
			...
			aria-live="polite"
			aria-atomic="true"
			class="pointer-events-auto flex items-center justify-between gap-4 rounded-lg p-4 shadow-lg {alert.severity ===
			'warning'
