...
const ROUTE = '/dev/form-primitive-preview';
...
test.describe('form primitive preview', () => {
	for (const theme of ['dark', 'light'] as const) {
		test.describe(theme, () => {
			test.beforeEach(async ({ page }) => {
				...
				await page.waitForSelector('[data-testid="form-primitive-preview-root"]');
			});

			for (const section of SECTIONS) {
				test(`${section.name} snapshot`, async ({ page }) => {
					const region = page.locator(`[data-testid="${section.id}"]`);
					await expect(region).toHaveScreenshot(`${theme}-${section.name}.png`, {
						threshold: 0.005
					});
				});
			}
