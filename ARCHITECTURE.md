...
Each notification crate exports a `static DESCRIPTOR: PluginDescriptor` generated by `declare_plugin!`.
The unified `PluginCatalog` is built from `all_descriptors()` in
`uptrakit-plugin-infrastructure-registry`; notification transport descriptors are feature-gated
(`notifications-webhook`, `notifications-telegram`, `notifications-email`) and instantiated as
singleton transports during catalog construction.
...
- **Main Helpers**: `init_crypto()`, `print_build_info()`, `default_resolve_shutdown()`, and
  `run_lifecycle_and_handle_errors()` reduce `main()` boilerplate. Tracing initialisation is provided by
  `uptrakit-tracing-init` (`TracingBuilder`, `init_cli_tracing`, `init_test_tracing`) and re-exported
  from this crate — binaries must not configure the global dispatcher themselves.
