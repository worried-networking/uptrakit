...
| `instance_host`            | `Option<String>`       | `#[serde(default, skip_serializing_if = "Option::is_none")]`                                                    | Optional string — the controller's public host (FQDN, optionally with port), sourced from the global `oauth.canonical_host` setting; absent when unset. Delivered to all services in `ServiceSettings`; currently only the agent's PVE infrastructure plugin consumes it (operator-facing annotations, e.g. PVE API token comments) |
...
The canonical source for capability wire strings and descriptions is the `Capability` enum in
`crates/shared/wire/src/capabilities.rs`. No capability list is hand-maintained in this document —
consult that enum directly for the current set of variants, their wire-string representation, and
