...
the `HostSoftwareItem` junction table, which stores the plugin config, package identifier, config
...
- **`host_software_items`**: junction table with composite PK `(host_id, software_item_id)`,
  `plugin_config_id` (FK → `plugin_configs.id`, NOT NULL), `package_identifier` (default `""`),
  `config_override?` (JSON), `installed_version?`, `installed_version_detected_at?`, `last_updated_at?`,
  `linked_at`. FKs on `host_id` and `software_item_id` cascade on delete; `plugin_config_id` uses
  ON DELETE RESTRICT.
  - Unique index: `uq_host_software_items_active ON (host_id, plugin_config_id, package_identifier)`
    — prevents the same (plugin, package) from being tracked twice on one host
  - Index: `idx_host_software_items_plugin_config_id`
...
- **`available_versions`**: `id` (UUID PK), `software_item_id` (FK → `software_items.id`, ON DELETE CASCADE),
  `version?`, `release_date?`, `release_notes?` (text), `extra?` (JSON — plugin-specific metadata such as tag,
...
- `HostSoftwareItem` belongs_to `PluginConfig` (many:1 — multiple host assignments can share one config)
- `PluginConfig` has_many `HostSoftwareItem`
- `SoftwareItem` has_many `AvailableVersion` (one:many — upstream release records per item)
- `package_identifier` distinguishes packages within a plugin config (e.g. different formulae from the same
  Homebrew config)
- `config_override` on the host assignment extends/overrides the base PluginConfig at resolution time (e.g.
...
| `plugin_types` | `Vec<String>` | Distinct plugin types across all host assignments |
...
| `latest_version` | `Option<String>` | Latest upstream version from `available_versions`, if known. Populated for agent-side plugins (Homebrew, PHS). `null` when not yet resolved. |
...
> plugin-specific (e.g. Homebrew may return `"1.2.3"`, PHS may return `"v1.2.3"`), no
> semver normalization is applied. For agent-side plugins (Homebrew, PHS), `latest_version`
> is populated by the agent during the `VersionCheckResults` handler. For controller-side
> plugins (GitHub Releases, Docker Registry), the value comes from the scheduled upstream
...
| POST | `/api/v1/software-items/{id}/hosts` | ManageSoftware | 200 | Assign to additional host(s); each assignment carries its own plugin config and package identifier |
| PUT | `/api/v1/software-items/{id}/hosts/{host_id}` | ManageSoftware | 200 | Update the plugin config, package identifier, or config override for a specific host assignment |
...
- Each host assignment must reference an active (non-deactivated) plugin config
...
- `(host_id, plugin_config_id, package_identifier)` must be unique across all host assignments — the same package
  cannot be tracked twice on one host via the same plugin
