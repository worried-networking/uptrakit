...
provider-action gate in `invoke_inner` (`crates/ui/surface-proxy/src/proxy.rs`): an interaction is
denied to a provider caller when it or its surface descriptor is action-gated, unless it sets
`provider_invocable`, since a service-initiated call carries no user to check the action against. See
