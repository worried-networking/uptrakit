...
**Why a new index, not expanding the existing one:**
The existing `uix_update_history_host_active` covers `(host_id)` for `pending` and
`in_progress`. Batch updates deliberately create multiple `Queued` rows for the same host
(different software items in sequence), so adding `queued` to a `(host_id)` index would
break batches. The new index constrains `(host_id, software_item_id)` — allowing batch
items on the same host for different software items to coexist as `Queued`, while preventing
two rows for the same software item/host pair in any non-terminal status.
...
(`update_triggers.rs:149`, before `has_active_update_for_host`) and return
`TriggerUpdateError::UpdateAlreadyActive` immediately if it returns `true`.

The DB unique index is belt-and-suspenders for concurrent requests; the explicit pre-check
gives the clean 409 in the normal single-controller case.
...
- `admin_events.rs`: update the manual `Deserialize` impl and the `all_variants()` test
  helper (which constructs `UpdateTriggered` without a `status` field and will fail to
  compile).
...
`trigger_status: TriggerUpdateStatus`. Pass `item.trigger_status.as_str().to_string()` as
`status` when constructing `AdminEvent::UpdateTriggered` at each site.
...
function applyUpdateTriggered(
  ...
  if (!detail) return;
  const host = detail.hosts.find((h) => h.host_id === data.host_id);
  if (!host) return;
  host.active_update_history_id = data.update_history_id;
  host.active_update_status = data.status;
  cache.set(data.software_item_id, detail); // trigger reactivity
}
```
...
`$state` variable rather than a `SvelteMap` — apply the same mutations directly to the
`$state` object.
...
| `crates/ui/web-api/src/actions/update_batches.rs`                      | Pass `item.trigger_status.as_str()` as `status` in both `UpdateTriggered` emit sites (lines 88, 331)               |
