...
Construction sites (lines ~842, ~898): `tenant_id` is not a free variable — extract it
from the enclosing function's parameters:

- Line ~842 (`prepare_pre_update_protection`): `target.tenant_id` where
  `target: &ValidatedUpdateTarget` (`ValidatedUpdateTarget.tenant_id: Uuid` is a public field).
- Line ~898 (`finalize_post_update_inner`): `record.tenant_id` where
  `record: &update_history::Model`.

`TenantDb::new` takes an owned `DatabaseConnection`, so clone the pool handle:

```rust
let tenant_db = TenantDb::new(db.clone(), target.tenant_id); // or record.tenant_id
let controller = QueryUpdateProtectionController::new(&tenant_db);
```

`DatabaseConnection` is a pool handle; `.clone()` is a cheap Arc clone.
...
registry re-exports, this import breaks. Fix: rewrite `build_settings_bag` to remove `EmailSmtpSettings` as an intermediate
typed struct. The raw DB queries (`load_settings_by_prefix`, `load_global_settings_by_prefix`)
are already in place — the rewrite replaces the `EmailSmtpSettings` normalization/mapping
step with direct `serde_json::Value` construction. The public signature
...
Follow the exact same dual-alias pattern as `MigrationsFn` (which has real and stub
definitions under `#[cfg(feature = "migrations")]` and `#[cfg(not(...))]` respectively,
so the `PluginDescriptor` field can be unconditionally present):
...
Add the field to `PluginDescriptor` — **no `#[cfg]` gate on the field itself**:

```rust
pub reset_tenant_data: Option<ResetTenantDataFn>,
```

The dual type alias above means `Option<ResetTenantDataFn>` is always defined
regardless of feature flags, so `#[cfg]` on the field is unnecessary and would break
the `declare_plugin!` macro (Rust does not allow `#[cfg]` attributes on struct literal
field expressions). `macro_rules!` struct literals cannot conditionally include or
exclude a field — the field must always be present in the literal.

All `PluginDescriptor` initializers are generated by the `declare_plugin!` macro.
Update the macro to accept an optional `reset_tenant_data: $reset_fn:expr` parameter
(following the `migrations:` optional parameter pattern already in the macro) and
emit `reset_tenant_data: $crate::__option_expr!( $( $reset_fn )? )` in the struct
literal — this sweeps all plugin crates atomically and defaults to `None`. Do NOT
add `#[non_exhaustive]` to `PluginDescriptor`: all construction goes through
`declare_plugin!` already, so `#[non_exhaustive]` would break the macro-generated
struct literals without providing any practical benefit.

### Proxmox plugin registration

`ProxmoxPlugin::DESCRIPTOR` is generated by `declare_plugin!`. Pass
`reset_tenant_data: proxmox_reset_tenant_data` to the macro (matching the
`migrations: migration_fn` optional-parameter pattern). The macro wraps it in
`Some(...)` via `__option_expr!`. Define `proxmox_reset_tenant_data` under BOTH `#[cfg]` branches — the real
implementation under `#[cfg(feature = "migrations")]`, and a no-op `fn()` stub under
`#[cfg(not(feature = "migrations"))]`. This mirrors the `MigrationsFn` dual-definition
pattern and lets `declare_plugin!` always receive
`reset_tenant_data: proxmox_reset_tenant_data` without requiring a duplicated macro
invocation.

`proxmox_reset_tenant_data` in the proxmox plugin (new `src/reset.rs` or inside
`controller_migration.rs`) deletes plugin-owned rows in FK-safe order. Required
imports in `reset.rs`: `sea_orm::{EntityTrait, QueryFilter, ColumnTrait, QuerySelect}`
— `QuerySelect` is needed for `.select_only()`, `.column()`, and `.into_tuple()`.
