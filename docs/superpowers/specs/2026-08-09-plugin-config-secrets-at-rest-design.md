...
- Expose a constructor for decoding a raw DB value with an explicit AAD (today `from_db` is
  `pub(crate)`), e.g. `EncryptedString::decode_db_value(raw: String, aad: &str) -> Result<Self>`.
- `encrypted_column!(TypeName, "uptrakit:<table>:<column>");` generates a newtype wrapping
  `EncryptedString` with:
  ...
  - constructor `new(plaintext_json: String) -> uptrakit_crypto::Result<Self>` and accessors
    `expose_secret()`, `as_json() -> Result<serde_json::Value>`;
  - `PartialEq` comparing **parsed JSON** (structural), not plaintext bytes — preserves the
    equality semantics of the current `serde_json::Value` columns (key order, whitespace, and the
    Postgres `jsonb → text` normalization must not create phantom diffs, e.g. in
    `queries/autodiscovery/default_configs.rs` change detection);
...
`routes/service_ws/handler/updates/replay.rs`.
...
- **Postgres:** `ALTER COLUMN ... TYPE text` for the three columns (`json`/`jsonb` → `text`).
  Use the typed `sea_query` alter-column builder. Plan-time verification item: confirm the cast is
  accepted without `USING` on a real Postgres (the Docker `database` integration suite); if a
  `USING` clause turns out to be required, that becomes a documented raw-SQL exception per
  `docs/development/database-migrations.md` with the inline comment naming the limitation.
- **SQLite:** no-op — `Json` columns are already stored as TEXT; no table recreation.
...
In `uptrakit-plugin-infrastructure-core`:

- `sensitive_paths(descriptor) -> Vec<String>` — dotted paths of fields with `sensitive == true`,
  collected from `config_form_fields()` (and `type_settings_form_schema()` where distinct).
...
- **Post-restore sentinel assertion:** after restore, if any sensitive path still equals `"***"`
  the write is rejected with a 400 validation error. This converts today's silent credential
  corruption (Docker auth variant-switch persists `"***"` verbatim; restore silently no-ops on
  deserialize failure) into a visible error. Consequence, documented in user-facing docs: a
  literal secret value `"***"` is unsettable.
...
**Parity gate before deletion:** a catalog-wide unit test asserts, for every plugin descriptor,
that the field set masked by the legacy `with_secrets_masked` on the sample config is a subset of
`sensitive_paths()`. This catches any config struct whose `SecretString` field lacks a
`.sensitive()` form-field declaration before the legacy path is removed (one-time gate; a
permanent test keeps asserting `SecretString`-bearing sample values never survive
`mask_present_keys`).
...
- **Startup sweep:** the reencrypt pass (§4) additionally counts existing layer-3 rows containing
  sensitive paths and warn-logs the count (single deployment: owner verifies zero; no destructive
...
2. **`notification_channels` restore-on-write** — verified missing: no `restore` call anywhere in
   ...
   `"***"` as the live credential. Fix with the same restore + sentinel-assert mechanism
   (channel plugins already declare masking via the notification plugin contract; wire the
   restore into the update path).
...
- Stamped (set to now) when:
  - **create**: the incoming config contains at least one sensitive path with a non-empty value;
...
3. Layer-3: HTTP override containing `auth_token` → 400; autodiscovery `config_override` with a
   sensitive key → stored stripped + warn; non-sensitive overrides unaffected.
4. `credential_updated_at`: stamped on secret create/change, untouched on non-secret update.
5. Sentinel assertion: update where restore cannot resolve `"***"` → 400 (both plugin config and
   notification channel variants).
6. Masker unit tests: sparse preservation, dotted paths, absent-path no-injection, parity gate
   (§5).
7. Migration `down()` on both backends: refuses with `ENC:` rows present, succeeds on
...
sensitive-path collection, fn-pointer retirement), `uptrakit-web-api-queries`,
...
(`EditHostAssignmentModal` sensitive-field filter; verify type-settings editor sentinel
round-trip).
...
- **New ADR** via `adrs new "Compile-time AAD encrypted column newtypes"` — records: newtype
  ...
  refuse-if-encrypted down() policy, and the intent to migrate existing registry columns to
  newtypes over time (out of scope here).
