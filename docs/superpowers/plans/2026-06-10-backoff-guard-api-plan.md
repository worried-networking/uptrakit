...
**Goal:** Replace `Backoff::next_delay()` / `Backoff::reset()` with a `#[must_use]` consuming `AttemptGuard` (verbs: `succeeded` / `partial_progress`
/ `failed`; warn-only `Drop` with **no state mutation**). Fix the enrollment-loop reset-on-partial-success bug at `lifecycle.rs:331` via splitting the
collapsed `is_receive_closed_report || is_transient_network_report` arm into two predicate-specific arms. Publish `uptrakit-backoff` 0.0.1 → 0.1.0 to
crates.io. Drop the duplicate copy in `service-sdk`. Audit the other 6 backoff consumers for analogous bugs and migrate.
...
- [ ] Implement guard methods (all consume `self`):
  - `pub fn sample_delay(&self) -> Duration` — read-only; samples `current + jitter`. Jitter re-sampled per call (document this; the `sample_` prefix
    is deliberate).
  - `pub fn succeeded(mut self)` — sets `resolved = true`, sets `self.backoff.current = self.backoff.base`.
  - `pub fn partial_progress(mut self)` — sets `resolved = true`, sets `self.backoff.current = self.backoff.base`. Same effect as `succeeded`;
    distinct verb to document partial-success classification at call site.
  - `pub fn failed(mut self)` — sets `resolved = true`, sets `self.backoff.current = (self.backoff.current * 2).min(self.backoff.max)`.
- [ ] Implement `Drop`:
...
- [ ] Document `AttemptGuard` with: the contract ("must be resolved"), the three verbs and when to use each (`succeeded` = work returned Ok;
      `partial_progress` = work returned Err but the cycle was healthy; `failed` = no progress made), and the Drop semantics.
- [ ] Document `sample_delay` warning: jitter re-sampled per call; store the result once. Example pattern.
- [ ] Document all three resolution methods (`succeeded`, `partial_progress`, `failed`) with one-paragraph rationale each.
- [ ] On `partial_progress`, include a `// WHY THIS EXISTS (do not collapse into succeeded)` block in the rustdoc citing the spec date
      (`2026-06-10-backoff-guard-api-design.md`) and the reasoning that the two verbs have identical state effect but encode different call-site
      intent. This is the canonical defense against a future "these are the same, let's merge them" PR — the comment forces the deletion PR to
      confront the original reasoning.
...
- [ ] Add new tests (spec §Tests list):
  ...
  - `bug_regression_partial_progress_at_cap_resets` — repeated `failed()` until `current == max`; then `partial_progress()`; then a fresh
    `attempt().sample_delay()` returns in the base+jitter range.
...
>
> **Audit decisions baked in**: every non-enrollment site below has its `partial_progress` vs `failed` verb pre-decided with stated rationale. Tasks
> include the literal `// <verb> chosen: <reason>` inline comment to leave in the code. Mqtt and nats use `failed()` uniformly — initial drafts
> proposed `had_connack`/`had_successful_fetch` flags, rejected because rumqttc/async_nats handle internal reconnect before surfacing Err and a
> reset-on-Err would stampede a recovering broker (same logic as the npm 5xx case). `partial_progress` is reserved for sites with an explicit
> close-code signal that the cycle was healthy — only the enrollment loop and the reconnect loop's `ReceiveClosed` arm qualify.
>
...
- [ ] In `crates/shared/service-sdk/src/lifecycle.rs` around lines 330–376:
  - Replace the collapsed `if is_receive_closed_report(&e) || is_transient_network_report(&e)` arm with two separate arms (this is the actual bug
    fix).
  - `is_receive_closed_report(&e)` arm: `let guard = enrollment_backoff.attempt(); let delay = guard.sample_delay(); guard.partial_progress();` then
    the interruptible sleep; `continue;`.
  - `is_transient_network_report(&e)` arm: identical structure but `guard.failed();`.
...
- [ ] Add classification test. **Location rule** (applies to every Phase 4 classification test): if the target file already has an inline
  ...
  - Assert that for `ReceiveClosed` the loop calls `partial_progress` (observable via `enrollment_backoff.attempt().sample_delay()` returning
    base-range value).
  - Assert that for a pre-upgrade `TransientNetwork` variant the loop calls `failed` (observable via cap-range value after escalation).
...
- [ ] In `crates/shared/service-sdk/src/lifecycle.rs` around lines 555–645:
  - **`Ok(outcome)` arm:** replace `reconnect_backoff.reset();` with `reconnect_backoff.attempt().succeeded();` on entry.
  - **`Err(e)` arm:** split `LoopError::TransientNetwork(_) | LoopError::ReceiveClosed` into two arms:
    - `LoopError::ReceiveClosed` → guard + `partial_progress()`.
    - `LoopError::TransientNetwork(_)` → guard + `failed()`.
  - **`LoopOutcome::Disconnected` arm** (line 625): replace `let delay = reconnect_backoff.next_delay();` with a two-guard pattern that preserves
    ...
    guard.succeeded();                // keep state at base — no escalation for clean disconnect
...
- [ ] Add classification test (same helper-enum trick if needed) for `LoopError::ReceiveClosed` → partial_progress; `LoopError::TransientNetwork` →
      failed.
...
- `failed()` for the poll-Err arm. No `had_connack` flag.
...
- [ ] Line 439 (`Packet::ConnAck` arm): replace `reconnect_backoff.reset();` with `reconnect_backoff.attempt().succeeded();`. ConnAck is the clean
      success signal; only the API form changes.
- [ ] Line 478 (poll-Err arm): replace `let delay = reconnect_backoff.next_delay();` with:

  ```rust
  let guard = reconnect_backoff.attempt();
  let delay = guard.sample_delay();
  // failed chosen: rumqttc retries connect internally; a surfaced Err means it gave up.
...
- [ ] Add classification test in the mqtt-runtime test module asserting `failed()` is the verb called on the poll-Err arm.

### Task 4.4 — Migrate `nats_transport.rs:201, 205`

**Audit decision** (pre-decided; document inline at migration site):

- `failed()` for the fetch-Err arm. No `had_successful_fetch` flag.
...
- [ ] Line 201 (Ok branch): replace `backoff.reset();` with `backoff.attempt().succeeded();`. Successful fetch is the clean success signal.
- [ ] Line 205 (Err branch): replace `let delay = backoff.next_delay();` with:

  ```rust
  let guard = backoff.attempt();
  let delay = guard.sample_delay();
  // failed chosen: async_nats handles internal reconnect; a surfaced fetch-Err means it gave up.
  // Resetting backoff would stampede a recovering JetStream server.
  guard.failed();
  ```

- [ ] Verify `cargo check -p uptrakit-web-api`.
- [ ] Add classification test asserting `failed()` is called on the Err branch.
...
- Use `failed()` for every attempt. Single verb keeps the bounded loop's intent obvious.
...
- [ ] In `crates/shared/nats/src/connection.rs` at the bounded loop (lines 56–74), restructure so the guard is constructed once at the top of each
  ...
  for attempt in 1..=MAX_ATTEMPTS {
      ...
      match async_nats::connect(url).await {
          Ok(c) => {
              guard.succeeded();      // resolve BEFORE the labeled break (footgun)
              break 'connect c;
          }
          Err(e) => {
              let delay = guard.sample_delay();
              guard.failed();         // resolve before sleep
...
- [ ] Add a `// failed chosen: bounded MAX_ATTEMPTS=10 retry; partial_progress vs failed has no behavioral difference here` comment.
- [ ] Verify `cargo check -p uptrakit-nats`.
- [ ] Add classification test asserting `failed()` is the verb called on the error branch.
...
- Two retry-eligible branches: request-failure at line 24 (TCP/TLS/DNS error from reqwest, pre-HTTP-response) and 5xx at line 49 (registry responded
  with server error).
- Both use `failed()`. The 5xx case may seem partial-progress flavored (the registry was reachable enough to send headers), but registries serve the
...
- [ ] In `crates/plugins/package-managers/npm/src/releases.rs`:
  - Line 24-38 (request-failure branch): wrap in guard pattern.
    `let guard = backoff.attempt(); let delay = guard.sample_delay(); guard.failed(); if attempt < FETCH_MAX_RETRIES { sleep(delay).await; }`. Add
    `// failed chosen: pre-HTTP transport error, fast-fail` comment.
  - Line 49-62 (5xx branch): same shape. Add
    `// failed chosen: 5xx burst signals registry overload/rate-limit; partial_progress would defeat the backoff hint` comment.
...
- [ ] Add classification test asserting both branches call `failed()`.
...
- Use `failed()` for every retryable attempt.
...
- [ ] In `crates/shared/agent-core/src/version_check.rs:543–555` (the `if retryable` branch):
  - Replace `let delay = backoff.next_delay();` with the guard pattern:
    `let guard = backoff.attempt(); let delay = guard.sample_delay(); guard.failed();`.
...
- [ ] Add `// failed chosen: PluginError trait surface carries no partial-progress signal` comment.
- [ ] Verify `cargo check -p uptrakit-agent-core`.
- [ ] Add classification test asserting `failed()` is called on a retryable `PluginError`.
...
**not** the realistic refactor pattern `let guard = backoff.attempt(); let delay = guard.sample_delay(); some_fallible_call()?; guard.failed();` — the
`?` between `attempt()` and resolution compiles cleanly because the guard is "used" later. In release the unresolved drop produces only a `warn!` log
and (per spec) **no state mutation**, so a CI grep nobody alerts on hides it. Spec stretch goal — promote to a required Phase 4 deliverable per user
request.
...
- [ ] Create `crates/core/functional-tests/tests/backoff_guard_no_question_in_attempt_scope.rs`. Test body:
  ...
  - Implement `syn::visit::Visit` with **full recursive delegation** (call `syn::visit::visit_expr(self, e)` / `visit_block` / etc. from each
    ...
    that misses nested scopes like `{ let g = b.attempt(); { some_fallible()?; g.failed(); } }`.
  - State machine: when `visit_local` sees an init whose RHS is `<expr>.attempt()` (matched via `ExprMethodCall` with `method.ident == "attempt"`),
    ...
    - the visitor encounters a method call `<binding_ident>.{succeeded,partial_progress,failed}(...)` — clean resolution;
...
- [ ] Verify the test FAILS when a deliberate violation is introduced (manual one-off check):
  - Temporarily add `let guard = backoff.attempt(); let _ = some_fallible()?; guard.failed();` to one migrated site.
...
- [ ] Replace the `next_delay()` / `reset()` example with the guard-pattern equivalent. Show:
  - Reconnect-with-reset-on-success shape:
    ...
    let delay = guard.sample_delay();
    match work().await {
        Ok(_) => { guard.succeeded(); break; }
        Err(_) => { guard.failed(); sleep(delay).await; }
    }
    ```

  - Partial-progress shape with the bug-fix framing: distinguish post-WS-upgrade close from pre-upgrade TCP refusal; use `partial_progress()` for the
    former.
...
- [ ] Regression A: kill the controller entirely → repeated connection-refused failures → delay must double up to 60s (`failed()` branch).
...
- [ ] Every audit decision is documented inline as a `//` comment at the migration site (`// <verb> chosen: <reason>`). All six non-enrollment audit
      decisions match the pre-decided verbs in this plan (mqtt: `failed` uniformly; nats_transport: `failed` uniformly; nats/connection: `failed`
      uniformly; npm: `failed` both branches; version_check: `failed`; reconnect loop at lifecycle.rs:481: ReceiveClosed → `partial_progress`,
      TransientNetwork → `failed`). Only the enrollment loop (lifecycle.rs:331) and the reconnect loop's ReceiveClosed arm use `partial_progress` —
      those are the sites with an explicit close-code-aware signal that the cycle was healthy.
- [ ] No `had_connack` / `had_successful_fetch` style session-state flags in mqtt or nats_transport — earlier drafts proposed them; final decision is
      `failed()` to preserve broker/server-friendly escalation hints during outages.
