...
| `frontend/src/lib/components/ui/SectionCard.svelte:17` | `rounded-2xl` → `rounded-[3px]` |
| `frontend/src/lib/components/ui/ProviderSelector.svelte:56` | `rounded-xl` → `rounded-[3px]` |
| `frontend/src/lib/components/ui/DataTable.svelte:76` | `px-4` → `px-[10px]` |
| `frontend/src/lib/components/ui/TabStrip.svelte:100,107-112` | outer `rounded-xl` → `rounded-[4px]`; button `rounded-lg` → `rounded-[3px]`; active state tint |
| `frontend/src/lib/components/ui/PageShell.svelte:25` | `text-3xl font-semibold tracking-tight` → `text-[20px] font-bold` |
| `frontend/src/lib/components/Modal.svelte:31` | `h3` class → explicit token class |
...
| `frontend/src/lib/components/surfaces/SurfaceSlot.test.ts:44` | update `section.card` selector to `data-ui` attr |
...
In `frontend/src/lib/components/ui/SectionCard.svelte` line 17, change:
...
- Modify: `frontend/src/lib/components/ui/DataTable.svelte:76`
...
Expected: no output from ProviderSelector test. DataTable test may have `px-4` references in tested content — examine any matches. These tests generally use semantic selectors (role, text), not className assertions; if `px-4` appears only in cell data strings it won't matter.
...
In `frontend/src/lib/components/ui/DataTable.svelte` line 76, change:
...
- Modify: `frontend/src/lib/components/ui/TabStrip.svelte:100,107-112`
...
3. Active state: `bg-[var(--accent)] text-[var(--text-inverted)] shadow-sm` → `bg-[rgba(var(--accent-rgb),0.12)] text-[var(--accent-bright)]` (tint per §4.11)

The existing Vitest tests do NOT assert on active state classes or radius — they use `data-state` attribute, keyboard navigation, and aria attributes. Verify this, then make the changes.
...
In `frontend/src/lib/components/ui/TabStrip.svelte` line 100, change:
...
In `frontend/src/lib/components/ui/TabStrip.svelte`, the button element starts at line 106. Find the class attribute that contains `rounded-lg` and the conditional active/inactive classes. Change:
...
Open a few regenerated PNGs (e.g. `ui-parity-software-page-chromium.png`) and confirm the active tab shows a tinted background with accent-colored text, not a solid filled tab.
...
Spec §2.4: h1 = `text-[20px] font-bold text-[var(--text-primary)]`. Current value: `text-3xl font-semibold tracking-tight text-[var(--text-primary)]`. The `text-[var(--text-primary)]` already matches — only size, weight, and tracking need updating.
...
- Modify: `frontend/src/lib/components/Modal.svelte:31`
...
In `frontend/src/lib/components/Modal.svelte` line 31, change:
...
- Modify: `frontend/src/lib/components/surfaces/SurfaceSlot.test.ts:44`
...
The existing test at line 44 uses `querySelectorAll('section.card')` to verify no surface items render when surfaces=[]. After our change, `.card` is removed from the section. The test would vacuously pass (0 matches = expected 0), but we update it to be meaningful using the new `data-ui` attribute.

- [ ] **Step 1: Update the test to use the new data-ui attribute**

In `frontend/src/lib/components/surfaces/SurfaceSlot.test.ts` line 44, change:
...
Expected: the "keeps structural slot container" test case PASSES (0 surface-slot-item found = 0 expected — vacuously true since attr doesn't exist). This is expected — we'll confirm the attr works when surfaces are present after implementing the change. Move to Step 3.
...
Spec: `h4` is not in §2.4. Map to h3 values per design decision Q5: `text-[13px] font-bold text-[var(--text-primary)]`. All five h4 elements in the merge preview section use `class="h4"`.
...
Replace each `class="h4"` with `class="text-[13px] font-bold text-[var(--text-primary)]"` for all 5 occurrences. They share the same replacement value so do a global replace:
