...
| --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PRAGMA <name> = <value>` (setter form) | SQLite pragma **writes** have no table-valued-function counterpart (`pragma_*` functions are read-only); used only by `#[cfg(test)]` helpers to seed rows without parents (`crates/shared/db/src/migration/mod.rs`, `crates/ui/web-api-auth/src/auth/device_flow.rs`) — production FK suspension uses `SqliteConnectOptions::foreign_keys(false)`, never a pragma |
