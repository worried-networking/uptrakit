...
**Terminology:** "bootstrap" in this section refers to the SSH-based host
bootstrap flow (`uptrakit-agent-ssh`), which provisions a _remote_ host over
an SSH session. This is distinct from the `bootstrap-host` CLI subcommand
described below, which provisions the host it runs _on_, locally — see
[Two writers, one implementation](#two-writers-one-implementation).
...
4. The resolved entries are handed to `uptrakit_agent_core::sudoers::write_sudoers_file`,
   which performs a validated activation sequence: it writes the content to a
   `<file>.tmp` temp file, `chmod`s it to `440`, validates it with `visudo -cf`,
   and only then `mv`s it into place at the final path. If any step fails, the
   temp file is removed and any existing drop-in is left untouched.
...
Exactly two code paths produce the sudoers drop-in: the SSH host sync flow
(`uptrakit-agent-ssh`) and the `bootstrap-host` CLI subcommand — available as
`uptrakit-agent bootstrap-host` and, behind the `embedded-agent` feature, as
`uptrakit-controller-standalone agent bootstrap-host`. Both route through the
same `write_sudoers_file` implementation, so the write/validate/activate
sequence above is identical regardless of which one ran.

They are **not** interchangeable, however. On SSH-managed PVE hosts, the sync
path additionally merges infra-contributed sudo entries (e.g. `pct exec *`,
`qm guest exec *`) into the same drop-in before the single write. A
`bootstrap-host` run on such a host regenerates the file from the plugin
registry only, which silently drops those infra lines until the next SSH
sync rewrites them.

**Ownership rule:** SSH sync owns the file on SSH-managed hosts;
`bootstrap-host` is for standalone/embedded-agent hosts. Hand-edited or
installer-templated sudoers files are no longer supported anywhere.

`bootstrap-host` must run as root, defaults to `--user uptrakit`, and derives
its grant list from each plugin's `required_sudo_commands()` the same way the
SSH sync path does — it never writes a blanket `NOPASSWD: ALL` grant.

To resolve each grant command's absolute path, `bootstrap-host` runs
`command -v <name>` under a `PATH` with the standard FHS directories
(`/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin`) prepended
ahead of the process's inherited `PATH`. This keeps grant paths stable across
console, cron, and systemd invocations; non-FHS layouts (e.g. NixOS) and
shims still resolve via the inherited tail.
