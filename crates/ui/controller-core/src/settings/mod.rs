...
use uptrakit_web_api_auth::SettingKey;
use uptrakit_web_api_auth::auth;
use uptrakit_web_api_auth::auth::authentication::AuthenticationSettings;
use uptrakit_web_api_auth::auth::registration::RegistrationSettings;
use uptrakit_web_api_auth::settings_store::{RawSettings, RawSettingsExt};
...
impl Settings {
    ...
    pub async fn load(
        ...
        let global_raw =
            uptrakit_web_api_auth::settings_store::load_all_global_settings(db).await?;
        let tenant_raw =
            uptrakit_web_api_auth::settings_store::load_all_settings(db, tenant_id).await?;
        ...
        let (version, global_version) =
            uptrakit_web_api_auth::settings_store::get_settings_versions(db, tenant_id).await?;
