...
/// # File-only keys
///
/// The following setting keys have been **removed** from this enum because they
/// moved to TOML config (spec §6.3, §20). Any code that previously used these
/// variants should reference the raw DB key string directly:
///
/// | Removed variant                  | DB key string                                    |
/// |----------------------------------|--------------------------------------------------|
/// | `TrustedProxies`                 | `"network.trusted_proxies"`                      |
/// | `RealIpHeader`                   | `"network.real_ip_header"`                       |
/// | `Sans`                           | `"network.sans"`                                 |
/// | `HttpsAddr`                      | `"network.https_addr"`                           |
/// | `ForwardedClientCertInfoHeader`  | `"network.forwarded_client_cert_info_header"`    |
/// | `ForwardedClientCertPemHeader`   | `"network.forwarded_client_cert_pem_header"`     |
/// | `PkiAddr`                        | `"network.pki_addr"`                             |
/// | `NatsUrl`                        | `"nats.url"`                                     |
/// | `ZeroconfEnabled`                | `"zeroconf.enabled"`                             |
/// | `ZeroconfUrl`                    | `"zeroconf.url"`                                 |
/// | `ZeroconfPkiAddr`                | `"zeroconf.pki_addr"`                            |
