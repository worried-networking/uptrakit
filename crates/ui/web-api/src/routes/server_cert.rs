...
async fn renew_server_certificate_inner(
    ...
    uptrakit_directories::write_secure_file_str(&key_tmp, &key_pem).await.map_err(|e| {
        report!(RenewCertError::CertWrite(std::io::Error::other(
            e.to_string()
        )))
    })?;
    uptrakit_directories::write_secure_file_str(&cert_tmp, &cert_pem).await.map_err(|e| {
        report!(RenewCertError::CertWrite(std::io::Error::other(
            e.to_string()
        )))
    })?;
