...
use uptrakit_internal_wire::{
    ...
    MqttReleaseTenantsPayload, MqttTenantConfig, OutgoingSeq, PingPayload, RegisterPayload,
...
// ---------------------------------------------------------------------------
// receive_first_service_message
// ---------------------------------------------------------------------------

/// Read the `Register` message sent by a connecting service.
///
/// All service types send `Register` immediately from `on_connected`, before
/// `ServiceSettings` is processed. The controller waits for this message on
/// every new authenticated connection.
///
/// Returns the [`RegisterPayload`] on success, or `None` when the connection
/// is closed or an unexpected message is received.
                ...
                match service_msg {
                    ...
                    _ => {
                        let err = ControllerMessage::Error(ErrorPayload {
                            ...
                            message: "expected Register message during session setup".to_string(),
                        });
