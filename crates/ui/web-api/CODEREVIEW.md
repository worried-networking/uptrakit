...
### M3. OIDC Role Sync Non-Atomic (Delete-Then-Insert Without Transaction) — FIXED
...
**Fix:** `sync_oidc_roles` delete-then-insert is now wrapped in a database transaction via `db.transaction()`. A failure during role insertion rolls back the delete, preserving the user's existing roles.
...
### M6. MQTT Password Stored in Plaintext — FIXED
...
**Fix:** MQTT passwords are now encrypted at rest via the `EncryptedString` SeaORM custom type (AES-256-GCM). Decryption occurs transparently when sending credentials to MQTT services over mTLS.
...
> Fix plans for open findings, ordered by severity. Plans 1-18 were implemented and removed. Plans 21 (M3), 24 (M6), and 27 (M9) are now also implemented.
...
### Plan 21: M3 — OIDC Role Sync Non-Atomic — IMPLEMENTED

Addressed by wrapping `sync_oidc_roles` delete-then-insert in a database transaction.
...
### Plan 24: M6 — MQTT Password Stored in Plaintext — IMPLEMENTED

Addressed via `EncryptedString` SeaORM custom type (AES-256-GCM encryption at rest).
...
### Plan 27: M9 — Relaxed Memory Ordering on Settings Version Counters — IMPLEMENTED (via H8)

Addressed as part of the H8 fix. Version counter loads now use `Ordering::Acquire`. The watch channel pattern eliminates inconsistent version pairs.
