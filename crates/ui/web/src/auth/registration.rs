...
/// Initialize registration settings on startup.
///
...
pub async fn initialize(db: &DatabaseConnection) -> Result<(RegistrationSettings, Option<String>)> {
...
/// Validate a registration attempt against current settings.
///
...
pub fn validate_registration(
    ...
    match settings.mode {
        ...
        RegistrationMode::Invite => {
            ...
            let stored_hash = settings.token_hash.as_deref().ok_or((
                http::StatusCode::FORBIDDEN,
...
/// After the first admin registers: set mode to Closed, clear token hash.
...
pub async fn complete_initial_setup(
    ...
    let mut state = registration.write().await;
    state.mode = RegistrationMode::Closed;
    state.token_hash = None;

    Ok(())
}

/// Admin action: update registration mode and optionally set a new token.
/// Updates both DB and in-memory state.
