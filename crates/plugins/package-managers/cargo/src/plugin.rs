...
impl Plugin for CargoPlugin {
    ...
    ///
    /// Runs `cargo install --list` and emits one [`DiscoveredSoftware`] entry per
    /// installed crate. Unlike system package managers, there are no infrastructure
    /// crates to filter out — everything in the `cargo install` list is user-installed.
    ///
    /// In discover-all mode (default config `{}`), emits one [`DiscoveryTarget`] per
    /// crate so the controller can auto-create a default Cargo plugin config and role
    /// assignments. When an explicit config is present (`include_prereleases` or
    /// `registry_url`), no targets are emitted — the config-ID path is used instead.
    #[tracing::instrument(skip_all)]
