...
///
/// PHS scripts always write their version file under `/root/.<slug>` because
/// they run as root via `pct exec`. The path is absolute and independent of
/// which user the agent process runs as.
///
...
/// escalation. The corresponding sudoers entry for `cat` is generated by
/// [`ProxmoxHelperScriptsPlugin::required_sudo_commands`] during host bootstrap.
///
/// `{package_identifier}` is the PHS slug (shell-escaped at runtime by the
/// Shell plugin's `detect_installed_version()` implementation).
const PHS_DETECT_VERSION_CMD: &str = "sudo cat -- /root/.{package_identifier}";
...
impl ProxmoxHelperScriptsPlugin {
    ...
    async fn try_read_version_file(&self, path: &str) -> Option<String> {
        let output = self
            .executor
            .execute_quiet(
                &CommandSpec::exec("cat", ["--".to_string(), path.to_string()]).privileged(),
            )
            .await
            .ok()?;
...
impl Plugin for ProxmoxHelperScriptsPlugin {
    ...
    fn required_sudo_commands(&self) -> Vec<SudoCommandEntry> {
        vec![SudoCommandEntry {
            command: "cat".into(),
            explanation: "Reading PHS version files from /root/.<slug> requires root privileges"
                .into(),
        }]
    }
    ...
    async fn discover_software(
        ...
        for script in &scripts {
            ...
            if let (Some(owner), Some(repo)) = (&analysis.github_owner, &analysis.github_repo) {
                // GitHub-managed: read version from /root/.<slug>.
                // PHS scripts execute via `pct exec` as root so they always
                // write the version file under /root/, regardless of the user
                // that runs the agent process.
                let version_path = format!("/root/.{}", script.slug);
...
mod tests {
    ...
    fn phs_shell_target_structure() {
        ...
        // Verify the version command reads from /root (not $HOME) with sudo.
        let version_cmd = target.plugin_config["version_command"]
            .as_str()
            .expect("version_command is a string");
        assert!(
            version_cmd.contains("/root/"),
            "version_command must read from /root/, got: {version_cmd}"
        );
        assert!(
    ...
    #[test]
    fn required_sudo_commands_declares_cat() {
        let plugin =
            ProxmoxHelperScriptsPlugin::new(ProxmoxHelperScriptsConfig::default(), test_executor())
                .expect("create");
        let entries = plugin.required_sudo_commands();
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].command, "cat");
