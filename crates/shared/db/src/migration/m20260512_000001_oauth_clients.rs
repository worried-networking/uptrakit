...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        // Partial index on active (non-revoked) clients.
        //
...
mod tests {
    ...
    fn migration_index() -> u32 {
        Migrator::migrations()
            .iter()
            .position(|m| m.name() == "m20260512_000001_oauth_clients")
    ...
    async fn oauth_clients_migration_round_trips() {
        ...
        db.execute_unprepared("SELECT COUNT(*) FROM oauth_clients")
            .await
        ...
        let res = db
            .execute_unprepared("SELECT COUNT(*) FROM oauth_clients")
            .await;
        assert!(
            res.is_err(),
