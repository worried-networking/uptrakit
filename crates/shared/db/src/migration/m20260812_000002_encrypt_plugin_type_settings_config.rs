...
/// Encrypt `plugin_type_settings.config` at rest.
///
/// `up()`:
/// - PostgreSQL: widen `config` from `json` to `text` so the entity's
///   [`uptrakit_shared_db::encrypted_columns::EncryptedPluginTypeConfig`]
    ...
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        if helpers::is_sqlite(manager) {
            ...
            manager
                .get_connection()
                .execute_unprepared(
                    "ALTER TABLE plugin_type_settings ALTER COLUMN config TYPE text USING config::text",
                )
                .await?;
        }
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        let encrypted_count: i64 = manager
            ...
            .query_one(
                &Query::select()
                    .expr_as(
                        Expr::col(PluginTypeSettings::Config).count(),
                        Alias::new("cnt"),
                    )
                    .from(PluginTypeSettings::Table)
                    .and_where(Expr::col(PluginTypeSettings::Config).like("ENC:%"))
                    .to_owned(),
            )
            .await?
        ...
        if encrypted_count > 0 {
            return Err(DbErr::Custom(
                "cannot revert: encrypted rows present in plugin_type_settings; decrypt is impossible in a migration (no DEK ring)".to_string(),
            ));
        }
        ...
        if !helpers::is_sqlite(manager) {
            ...
            manager
                .get_connection()
                .execute_unprepared(
                    "ALTER TABLE plugin_type_settings ALTER COLUMN config TYPE json USING config::json",
                )
                .await?;
        }
...
mod tests {
    ...
    /// `plugin_type_settings.tenant_id` are satisfied.
    ...
    async fn seed_plugin_type_setting(db: &DatabaseConnection, plugin_type: &str, config: &str) {
        db.execute(
            &Query::insert()
    ...
    async fn up_then_down_on_plaintext_succeeds() {
        ...
        seed_plugin_type_setting(&db, "package-manager.apt", r#"{"foo":"bar"}"#).await;
        ...
        let row = db
            .query_one(
                &Query::select()
                    ...
                    .from(Alias::new("plugin_type_settings"))
                    .and_where(Expr::col(Alias::new("plugin_type")).eq("package-manager.apt"))
                    .to_owned(),
            )
            .await
    ...
    async fn down_refuses_with_enc_rows() {
        ...
        seed_tenant(&db).await;
        seed_plugin_type_setting(&db, "package-manager.apt", "ENC:v3:deadbeef").await;
