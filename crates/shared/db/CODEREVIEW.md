...
| DB-05 | Low | Code Quality | String-typed columns that could use `DeriveActiveEnum` for type safety: `mqtt_client.transport` (`:11`), `mqtt_client.connection_status` (`:18`), `session.token_type` (`:14`), `pending_device_flow.status` (`:12`), `update_output_line.stream` (`:10`). | Multiple entity files |
