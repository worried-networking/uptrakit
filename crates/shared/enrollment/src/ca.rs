...
//! 4. If `--tofu`: fetch via HTTPS (accept any cert)
...
/// 4. `--tofu` → fetch from base URL with insecure TLS, save to identity
...
pub async fn bootstrap_ca(
    ...
    // 4. TOFU: fetch from controller URL with insecure TLS
    if tofu {
        tracing::info!("TOFU: fetching CA (accepting any server certificate)");
        let pem = fetch_ca_certificate(base_url, CaTlsMode::Insecure).await?;
        identity
            .save_ca_cert(&String::from_utf8_lossy(&pem))
            .await?;
        tracing::info!("CA certificate saved to disk");
