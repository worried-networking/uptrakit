    ...
    fn build_env_filter(&self) -> EnvFilter {
        ...
        if let Some(directives) = self.directives.get(&clamped) {
            for (target, level) in directives {
                ...
                #[expect(
                    clippy::expect_used,
                    reason = "infallible: directive string is constructed from compile-time constants; a parse failure indicates a programming error"
                )]
...
pub fn init_test_tracing() {
    ...
    INIT.call_once(|| {
        ...
        let warn_directive = "warn".parse().expect("valid directive");
        let mut filter = EnvFilter::new("").add_directive(warn_directive);
        if let Ok(rust_log) = std::env::var("RUST_LOG") {
            for part in rust_log.split(',') {
                let part = part.trim();
                if part.is_empty() {
        ...
        #[expect(
            ...
            reason = "try_init() returns Err if a subscriber is already set; second call is intentionally ignored"
        )]
        tracing_subscriber::fmt()
            .with_test_writer()
            .with_env_filter(filter)
