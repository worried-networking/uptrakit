...
//!
//! Tracing subscriber initialization is intentionally **not** provided here.
//! Libraries must not configure the global tracing dispatcher; each binary
//! is responsible for calling `tracing_subscriber::fmt().init()` (or
//! equivalent) in its own `main()`.
