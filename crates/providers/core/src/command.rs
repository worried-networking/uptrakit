...
pub async fn run_command_exec(
    ...
    let mut child = cmd
        .spawn()
        .map_err(|e| report!(ProviderError::CommandSpawn(e)))?;

    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| report!(ProviderError::CaptureFailed("stdout".to_string())))?;
    let stderr = child
        .stderr
        .take()
        .ok_or_else(|| report!(ProviderError::CaptureFailed("stderr".to_string())))?;
    ...
    let status = child
        .wait()
        .await
        .map_err(|e| report!(ProviderError::CommandWait(e)))?;
...
mod tests {
    ...
    async fn test_run_command_failure() {
        ...
        assert!(
            matches!(err.current_context(), ProviderError::CommandFailed(1)),
    ...
    async fn test_run_command_with_shell_failure() {
        ...
        let err = result.unwrap_err();
        assert!(
            matches!(err.current_context(), ProviderError::CommandFailed(42)),
