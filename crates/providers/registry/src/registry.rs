...
/// Generates the four `ProviderRegistry` dispatch methods from a single
...
///
/// All four dispatch methods (`create_provider`, `validate_config`,
/// `mask_config_secrets`, `restore_config_secrets`) are generated by the
/// [`register_providers!`] macro from a single declaration. To add a new
/// provider, add one line to the macro invocation below.
