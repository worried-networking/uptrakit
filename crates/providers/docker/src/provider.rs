...
impl Provider for DockerProvider {
    ...
    async fn fetch_releases(
        ...
        let ir: ImageRef = package_identifier
            ...
            .map_err(|e: crate::image_ref::ParseImageRefError| {
                uptrakit_provider_core::ProviderError::ProviderInternal(e.to_string())
            })?;
    ...
    async fn detect_installed_version(
        ...
        let ir: ImageRef = package_identifier
            .parse()
            .map_err(|e: crate::image_ref::ParseImageRefError| {
                ProviderError::ProviderInternal(e.to_string())
            })?;
        ...
        match self.docker_client.inspect_image(&full_ref).await {
            ...
            Ok(None) => Ok(None),
            Err(e) => Err(uptrakit_provider_core::ProviderError::ProviderInternal(
                e.to_string(),
            )
            .into()),
        }
...
mod tests {
    ...
    impl CommandExecutor for MockCommandExecutor {
        ...
        ) -> uptrakit_command::Result<uptrakit_provider_core::CommandOutput> {
            Ok(uptrakit_provider_core::CommandOutput {
                output: String::new(),
                exit_code: 0,
            })
        }
    ...
    fn provider_creation_succeeds_with_empty_config() {
        ...
        assert!(DockerProvider::new_for_test(config, test_executor(), default_mock_client()).is_ok());
    }
    ...
    fn provider_creation_fails_with_invalid_regex() {
        ...
        assert!(DockerProvider::new_for_test(config, test_executor(), default_mock_client()).is_err());
    }

    #[test]
    fn capabilities_includes_discover_local_software() {
        let provider = DockerProvider::new_for_test(
    ...
    async fn detect_installed_version_semver_mode_returns_none() {
        ...
        let provider = DockerProvider::new_for_test(config, test_executor(), default_mock_client()).unwrap();
    ...
    async fn detect_installed_version_digest_mode_returns_digest() {
        ...
        let provider = DockerProvider::new_for_test(config, test_executor(), mock).unwrap();
        let result = provider.detect_installed_version("nginx").await.unwrap();
    ...
    async fn execute_update_calls_docker_client() {
        ...
        let provider = DockerProvider::new_for_test(DockerConfig::default(), test_executor(), mock)
            .expect("valid config");
    ...
    async fn execute_update_with_post_pull_command() {
        ...
        let provider = DockerProvider::new_for_test(config, test_executor(), mock)
            .expect("valid config");
        let (tx, mut rx) = mpsc::channel(100);
        let result = provider
            .execute_update("nginx", "1.25.0", None, &tx)
    ...
    async fn execute_update_with_compose_restart() {
        ...
        let provider = DockerProvider::new_for_test(config, mock_executor(), mock)
            .expect("valid config");
        let (tx, mut rx) = mpsc::channel(100);
        let result = provider
            .execute_update("nginx", "1.25.0", None, &tx)
    ...
    async fn discover_software_groups_by_image() {
        ...
        let provider = DockerProvider::new_for_test(DockerConfig::default(), test_executor(), mock).unwrap();
