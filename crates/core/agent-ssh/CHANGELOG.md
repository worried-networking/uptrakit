...
## [0.0.3](https://github.com/worried-networking/uptrakit/compare/uptrakit-agent-ssh-v0.0.2...uptrakit-agent-ssh-v0.0.3) - 2026-05-18

### Added

- *(agent-ssh-runtime)* move db entities, migrations, and SSH logic modules from agent-ssh
- *(agent-ssh)* select RouterOsHostRuntime or StandardHostRuntime per host in client.rs
- *(agent-ssh)* auto-detect RouterOS in bootstrap_connect; route to RouterOS bootstrap
- *(agent-ssh)* add RouterOS bootstrap plan + execute (bootstrap_routeros.rs)
- *(agent-ssh)* add collect_remote_host_info_routeros and machine-ID extraction helpers
- *(agent-ssh)* add routeros_host_config SeaORM entity
- *(agent-ssh)* add routeros_host_config migration
- *(agent-ssh)* add RouterOsSshExecutor with typed RouterOS CLI methods
- *(agent-ssh)* add SFTP sftp_put/sftp_remove to SshSession and SshCommandExecutor
- *(plugins)* gate instance-scoped plugin construction by snapshot
- *(agent-ssh-runtime)* override service_migrations() on AgentSshHandler; remove transitional free fn
- *(agent-ssh-runtime)* add crypto helpers, AgentSshHandler, AgentSshMode, EciesKeypair, service_migrations()
- *(agent-ssh-runtime)* expand deps for incoming module migration
...
- *(agent-ssh)* commit missing routeros_host_config entity and migration files
...
- *(agent-ssh)* remove orphaned commands/ directory
- *(agent-ssh)* thin to binary shell, delegate all logic to runtime
- *(agent-ssh)* migrate SshAgentHandler to dyn ServiceTransport
- *(agent-core)* public functions accept Arc<dyn HostRuntime> instead of Arc<dyn CommandExecutor>
- *(agent-ssh)* split SshCommandExecutor into base + PosixSshCommandExecutor
- *(pki)* migrate service-sdk, cli, and integration-test keygen helpers to P-384
- *(agent-core)* update.rs accepts Arc<dyn HostRuntime> instead of Arc<dyn CommandExecutor>
- *(agent-core)* version_check accepts Arc<dyn HostRuntime> instead of Arc<dyn CommandExecutor>
- *(agent-core)* update module doc comment to reflect resumable post-hook scheduling
...
## [0.0.2](https://github.com/worried-networking/uptrakit/compare/uptrakit-agent-ssh-v0.0.1...uptrakit-agent-ssh-v0.0.2) - 2026-05-05

### Added

- *(agent-ssh)* apply icons to SSH host actions and forward to interactions
