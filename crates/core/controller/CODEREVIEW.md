...
| --- | --- | --- | --- | --- | --- |
| ~~C-2~~ | ~~Low~~ | ~~Safety~~ | ~~`startup.rs:436-442`~~ | ~~`.map_err(\|_\| ...)` discards the original `AddrParseError` when parsing `DEFAULT_HTTPS_ADDR`.~~ | ~~Preserve the original error in the message.~~ **FIXED:** Changed `.map_err(\|_\| ...)` to `.map_err(\|e\| ...)` and included the original error in the message via `format!("invalid default HTTPS address constant: {e}")`. |
| ~~C-3~~ | ~~Info~~ | ~~Code Quality~~ | ~~`pki.rs:900,904`~~ | ~~`.unwrap_or([0; 4])` / `.unwrap_or([0; 16])` on `try_into()` after the `match ip_bytes.len()` already guarantees the slice length. The fallback is unreachable but reads as though `0.0.0.0` / `::` is a valid SAN.~~ | ~~Replace with `.expect("length already matched")` (acceptable in this context since it is truly unreachable), or restructure using `TryFrom` with an explicit error.~~ **FIXED:** Replaced `unwrap_or([0; N])` with `if let Ok(octets)` pattern to skip invalid entries instead of inserting bogus IPs. |
