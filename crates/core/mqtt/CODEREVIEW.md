...
`uptrakit-mqtt` has solid recovery behavior for broker reconnects: cached state is republished, HA discovery is rebuilt, and the service keeps explicit per-tenant caches instead of relying on broker state alone. The main remaining issue is that observability events are still lossy under backpressure.

## Strengths

- Reconnect handling republished cached software, host, and connectivity state instead of assuming retained topics are authoritative.
