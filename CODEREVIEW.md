...
without catch-all variants (now carry an `Unknown` serde catch-all), and the manually maintained
`validate_package_identifier` (now auto-generated by the `register_plugins!` macro) have been
fixed.
